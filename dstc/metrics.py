...
def calc_nist_bleu(path_refs, path_hyp, fld_out='temp', n_lines=None):
	...
	_write_xml([''], f'{fld_out}/src.xml', 'src', n_lines=n_lines)
	_write_xml([path_hyp], f'{fld_out}/hyp.xml', 'hyp')
	_write_xml(path_refs, f'{fld_out}/ref.xml', 'ref')

	time.sleep(1)
	cmd = [
		'perl',
		'3rdparty/mteval-v14c.pl',
		'-s',
		f'{fld_out}/src.xml',
		'-t',
		f'{fld_out}/hyp.xml',
...
def calc_meteor(path_refs, path_hyp, fld_out='temp', n_lines=None, pretokenized=True):
	...
	makedirs(fld_out)
	path_merged_refs = f'{fld_out}/refs_merged.txt'
